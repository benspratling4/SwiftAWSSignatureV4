...
extension URLRequest {
	...
	mutating func sign(for AWSAccount:AWSAccount, now:Date, signPayload:Bool = false) {
		...
		//auth header
		let header = newAuthorizationHeader(AWSAccount: AWSAccount, now: now, nowComponents: nowComponents)
	...
	func canonicalRequest(now:Date)->(request:String, signedHeaders:String)? {
		...
		guard let encodedURI:String = uriString.uriEncoded(encodeSlash: false) else { return nil }
		if let queryLongString = queryString {
			...
			let reconstituted:[String] = queryItems.map{
				$0.components(separatedBy: "=")
					.flatMap{$0.uriEncoded(encodeSlash: true)}
				.joined(separator: "=")}
		...
		let hashedBody:String = sha256HashedBody.map { CryptoUtils.hexString(from: $0) }
	...
	func stringToSign(AWSAccount:AWSAccount, now:Date, nowComponents:DateComponents)->(string:String, signedHeaders:String)? {
		let timeString:String = HTTPDate(now: nowComponents)
		guard let (request, signedHeaders) = canonicalRequest(now:now) else { return nil }
	...
	func newAuthorizationHeader(AWSAccount:AWSAccount, now:Date, nowComponents:DateComponents)->String? {
		guard let signingKey:[UInt8] = AWSAccount.keyForSigning(now:nowComponents)
			,let (string, signedHeaders) = stringToSign(AWSAccount:AWSAccount, now:now, nowComponents:nowComponents)
			else { return nil }
